...
function getIssueResponses(issue) {
    const responses = responses_1.Responses.map(r => r.response({ issue })).filter(r => r !== undefined);
    if (responses.length === 0) {
        return undefined;
    }
    return responses;
}
exports.getIssueResponses = getIssueResponses;
run();
